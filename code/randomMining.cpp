...
//	check connectedness
	std::cout<<"File reading time: "<<filereadtime<<" sec"<<std::endl;	
	infile.close();
}
...
void randommining::parseArgs(int argc, char* argv[]) {
      ...
      if(((string)(argv[1]))!="-d" || ((string)(argv[3]))!="-i" || ((string) (argv[5]))!="-s" || ((string) (argv[7]))!="-step"){
	      printUsage(argv[0]);
	      exit(0);
      }
      ...
      for ( i=1; i < argc; i++){
		      ...
		      else if( ((string)(argv[i])).compare("-step")==0){
				      stepSize=boost::lexical_cast<int>(argv[++i]);
		      }
	}
}
